...
Worker.prototype.experience = function () {//  расчет опыта
    Worker.allWorkers.push(this);
    return (this.startedToWork + (new Date().getMinutes() - this.nextExperienceInMinutes));
};
Worker.prototype.coefExperience = function () {// коэффициент опыта
...
Worker.prototype.coefSalary = function () {     //  расчет коэффициента зарплаты
...
Worker.prototype.salary = function () {       //  расчет зарплаты
    return (this.coefExperience()*this.coefSalary()*AVERAGE_SALARY).toFixed(0);
};
Worker.prototype.changeProfession = function (profession) {
