...
Worker.AVERAGE_SALARY = 3000;      //  средняя зарплата
...
function Worker(obj) {
    ...
    this.experience = function () {     //  расчет опыта
        return (new Date().getFullYear() - this.startedToWork.getFullYear()) + (new Date().getMinutes() - this.nextExperienceInMinutes);
    };
    ...
    this.salary = function () {       //  расчет зарплаты
        return (this.coefExperience()*this.coefSalary()*Worker.AVERAGE_SALARY).toFixed(0);
    };
