...
// Worker.showWorkersByProfession = function () {};
// Worker.showWorkersBySurname = function () {};
// Worker.showWorkersBySalary = function () {};
// Worker.showWorkersByExperience = function () {};
// Worker.showBestWorkersSalary = function () {};
// Worker.showBestWorkersSalaryByProfession = function () {};
...
function Worker(obj) {
    ...
    this.startExperienceInMinutes = new Date().getMinutes();
    this.experience = function () {     //  расчет опыта
        return (new Date().getFullYear() - this.startedToWork.getFullYear()) + (new Date().getMinutes() - this.startExperienceInMinutes);
    };
    ...
    function salaryAllWorker() {
        var arraySalaryAllWorkers = 0;
        for(var i=0; i<Worker.allWorkers.length;i++){
            arraySalaryAllWorkers += arraySalaryAllWorkers[i];
        }return arraySalaryAllWorkers;
    }
...
Worker.avarageSalary = function () {
    var arraySalaryWorkers = [];
    for(var i=0; i<Worker.allWorkers.length;i++){
        arraySalaryWorkers.push(+Worker.allWorkers[i][4])
    }return salaryAllWorker();
};

// Worker.avarageSalaryByProfession = function (argument) {
//     var arraySalaryWorkers = [];
//     for(var i=0; i<Worker.allWorkers.length;i++){
//         if(Worker.allWorkers[i][2] === argument){
//             arraySalaryWorkers.push(Worker.allWorkers[i]);
//         }
//     }return function () {
//         var arraySalaryAllWorkersByProfession = 0;
//         for(var i=0; i<Worker.allWorkers.length;i++){
//             arraySalaryAllWorkersByProfession += arraySalaryAllWorkersByProfession[i];
//         }return arraySalaryAllWorkersByProfession;
//     }
